...
- **Operaciones básicas:**
  + asignación (`:=`), suma (`+`), resta (`-`), multiplicación (`*`), división (`/`, `DIV`), menor (`<`), mayor(`>`) e igual(`=`) valen **1 OE**.
...
> [!WARNING]
> Consulten si `<=`, `=>` y `=` valen **1 OE** o **2 OE** especificamente para esta materia. En realidad, lo ideal sería que consulten y duden de todo, principalmente en esta sección que muchas veces varía de acuerdo a la bibliografía y/o convennciones que se tengan.
